...
static int buttons_hypernova_start_filter_exec(bContext *C, wmOperator * /*op*/)
{
  SpaceHypernova *space = CTX_wm_space_hypernova(C);
...
void buttons_hypernova_OT_start_filter(wmOperatorType *ot)
{
  ...
  ot->poll = ED_operator_buttons_hypernova_active;
}

static int buttons_hypernova_clear_filter_exec(bContext *C, wmOperator * /*op*/)
{
  SpaceHypernova *space = CTX_wm_space_hypernova(C);
...
static int toggle_pin_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  PointerRNA sbuts_ptr = RNA_pointer_create(&screen->id, &RNA_SpaceHypernova, sbuts);

  /* Create the new ID pointer and set the pin ID with RNA
   * so we can use the property's RNA update functionality. */
  ID *new_id = (sbuts->flag & SB_PIN_CONTEXT) ? buttons_hypernova_context_id_path(C) : nullptr;
