...
static SpaceHypernova *find_space_properties(const bContext *C);

/************************* Texture User **************************/

static void buttons_hypernova_texture_user_socket_property_add(ListBase *users,
                                                     ID *id,
                                                     PointerRNA ptr,
                                                     PropertyRNA *prop,
                                                     bNodeTree *ntree,
...
static void buttons_hypernova_texture_user_property_add(ListBase *users,
                                              ID *id,
                                              PointerRNA ptr,
                                              PropertyRNA *prop,
                                              const char *category,
...
static void buttons_hypernova_texture_user_node_add(ListBase *users,
                                          ID *id,
                                          bNodeTree *ntree,
                                          bNode *node,
                                          PointerRNA ptr,
...
static void buttons_hypernova_texture_users_find_nodetree(ListBase *users,
                                                ID *id,
                                                bNodeTree *ntree,
                                                const char *category)
{
  if (ntree) {
    LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
      if (node->type == CMP_NODE_TEXTURE) {
        ...
        buttons_hypernova_texture_user_node_add(
        ...
        PointerRNA ptr = RNA_pointer_create(&ntree->id, &RNA_Node, node);
        buttons_hypernova_texture_user_node_add(users,
                                      id,
                                      ntree,
                                      node,
                                      {nullptr},
        ...
        buttons_hypernova_texture_users_find_nodetree(users, id, (bNodeTree *)node->id, category);
      }
    }
  }
}

static void buttons_hypernova_texture_modifier_geonodes_users_add(
    Object *ob,
  ...
  for (bNode *node : node_tree->all_nodes()) {
    if (node->type == NODE_GROUP && node->id) {
      if (handled_groups.add(reinterpret_cast<bNodeTree *>(node->id))) {
        ...
        buttons_hypernova_texture_modifier_geonodes_users_add(
    ...
    LISTBASE_FOREACH (bNodeSocket *, socket, &node->inputs) {
      ...
      if (tex != nullptr) {
        buttons_hypernova_texture_user_socket_property_add(users,
                                                 &ob->id,
                                                 ptr,
                                                 prop,
                                                 node_tree,
...
static void buttons_hypernova_texture_modifier_foreach(void *user_data,
                                             Object *ob,
                                             ModifierData *md,
                                             ...
                                             PropertyRNA *texture_prop)
{
  ...
  if (md->type == eModifierType_Nodes) {
    ...
    if (nmd->node_group != nullptr) {
      blender::Set<const bNodeTree *> handled_groups;
      buttons_hypernova_texture_modifier_geonodes_users_add(ob, nmd, nmd->node_group, users, handled_groups);
    }
  }
  else {
    const ModifierTypeInfo *modifier_type = BKE_modifier_get_info((ModifierType)md->type);

    buttons_hypernova_texture_user_property_add(
        users, &ob->id, *ptr, texture_prop, N_("Modifiers"), modifier_type->icon, md->name);
  }
}

static void buttons_hypernova_texture_users_from_context(ListBase *users,
                                               const bContext *C,
                                                         SpaceHypernova *sbuts)
{
  ...
  if (scene && scene->nodetree) {
    buttons_hypernova_texture_users_find_nodetree(users, &scene->id, scene->nodetree, N_("Compositor"));
  }

  if (linestyle && !limited_mode) {
    buttons_hypernova_texture_users_find_nodetree(
  ...
  if (ob) {
    ...
    /* modifiers */
    BKE_modifiers_foreach_tex_link(ob, buttons_hypernova_texture_modifier_foreach, users);
    ...
    if (psys && !limited_mode) {
      for (a = 0; a < MAX_MTEX; a++) {
        ...
        if (mtex) {
          ...
          buttons_hypernova_texture_user_property_add(users,
                                            &psys->part->id,
                                            ptr,
                                            prop,
                                            N_("Particles"),
...
void buttons_hypernova_texture_context_compute(const bContext *C, SpaceHypernova *sbuts)
{
   ...
   * properties editor, before the buttons_hypernova are created. */
  ...
  buttons_hypernova_texture_users_from_context(&ct->users, C, sbuts);
...
static void template_texture_select(bContext *C, void *user_p, void * /*arg*/)
{
  ...
  SpaceHypernova *sbuts = find_space_properties(C);
...
{
  /* callback when opening texture user selection menu, to create buttons_hypernova. */
...
void uiTemplateTextureShowHypernova(uiLayout *layout, const bContext *C, PointerRNA *ptr, PropertyRNA *prop)
{
  ...
  SpaceHypernova *sbuts_context = CTX_wm_space_hypernova(C);
