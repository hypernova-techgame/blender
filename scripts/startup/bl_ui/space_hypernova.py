...
class HYPERNOVA_HT_header(Header):
    bl_space_type = 'HYPERNOVA'
    ...
    def draw(self, context):
        ...
        layout.popover(panel="HYPERNOVA_PT_options", text="")


class HYPERNOVA_PT_navigation_bar(Panel):
    bl_space_type = 'HYPERNOVA'
    ...
    def draw(self, context):
        ...
        if view.search_filter:
            layout.prop_tabs_enum(
                view, "context_hypernova", data_highlight=view,
                property_highlight="tab_search_results", icon_only=True,
            )
        else:
            layout.prop_tabs_enum(view, "context_hypernova", icon_only=True)


class HYPERNOVA_PT_options(Panel):
    """Show options for the HYPERNOVA editor"""
    bl_space_type = 'HYPERNOVA'
...
class HYPERNOVAAnimationMixin:
    """Mix-in class for Animation panels.

    This class can be used to show a generic 'Animation' panel for IDs shown in
    the HYPERNOVA editor. Specific ID types need specific subclasses.

    For an example, see DATA_PT_camera_animation in HYPERNOVA_data_camera.py
    """
    bl_space_type = 'HYPERNOVA'
    ...
    bl_order = PropertyPanel.bl_order - 1  # Order just above the Custom HYPERNOVA.
    ...
    def draw_action_and_slot_selector(cls, context, layout, animated_id):
        if not animated_id:
            ...
            print("HYPERNOVAAnimationMixin: no animatable data-block, this is a bug "
...
classes = (
    HYPERNOVA_HT_header,
    HYPERNOVA_PT_navigation_bar,
    HYPERNOVA_PT_options,
)
